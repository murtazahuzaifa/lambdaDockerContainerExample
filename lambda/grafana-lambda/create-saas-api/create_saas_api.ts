...
exports.handler = async (event: any) => {
    ...
    const { apiId, email } = event.data;
    const encoding = Base64.encode(`${process.env.USERNAME}:${process.env.PASSWORD}`)
    console.log(encoding);

    const split_api = apiId.split(".");

    try{

        // const check_user = await grafana.instance.get(`/api/users/lookup?loginOrEmail=${email}`, {
        ...
        ///Create Tenant
        const creat_org = await grafana.instance.post("/api/orgs", {name : `SAAS#${apiId}`}, {
            headers: {'Authorization': `Basic ${encoding}`}
        })
        ...
        /// Add user tot tenant
        const add_user_to_org = await grafana.instance.post(`/api/orgs/${creat_org.data.orgId}/users`, {loginOrEmail: email, role: "Viewer"}, {
            headers: {'Authorization': `Basic ${encoding}`}
        })

        console.log('GRAFANA ADDed', add_user_to_org.data);
